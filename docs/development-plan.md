...
|---|---|---|
| I01 | 统一工具链和固定依赖 | Node 24、根命令、锁文件 |
| I02 | 修复离线 E2E 和本地质量命令 | lint/typecheck/test/integration/build |
| I03 | GitHub 最小门禁 | Actions、模板、标签、保护 |
| I04 | 判别式云函数响应 | prepare/confirm/advice union |
| I05 | 模糊路线确认闭环 | 服务端 ID + 前端确认 |
| I06 | 确定性安全合并 | AI 不可覆盖规则项 |
| I07 | 领域模型与旧数据适配 | Place/Route/Variant schema |
| I08 | 武功山数据 | 可追溯 verified variant |
| I09 | 四姑娘山二峰数据 | climb variant |
| I10 | 五台山数据 | 小朝台 + blocked 大朝台 |
| I11 | 玉龙雪山数据 | 4680 tour variant |
| I12 | 贡嘎数据 | 三日点到点 variant |
| I13 | 稳定 ID 搜索解析 | 地点/路线/变体区分 |
| I14 | 多点小时天气 | 活动窗口契约 |
| I15 | TP-VERDICT-1 | 纯规则和原因码 |
| I16 | 攀登支持、日落和数据不足 | 输入与组合规则 |
| I17 | 服务端 TripContext | queryId + 所有权/过期 |
| I18 | 移除可信 baseData 回传 | advice 仅 queryId |
| I19 | 私人历史和停用 UGC | read/delete/clear only |
| I20 | 前端 reducer 与服务层 | 显式状态和竞态保护 |
| I21 | 搜索确认输入流程 | variant/date/time/support |
| I22 | 结果体验 | verdict/hourly/checklist/sources |
| I23 | 降级与恢复 | weather/AI/history 独立恢复 |
| I24 | Beta 综合验证 | 回归、构建、人工清单、文档 |
| I25 | Goal 最终 Review | 完成报告和验收结论 |
