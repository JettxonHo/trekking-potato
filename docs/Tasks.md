...
- [x] **Task**: 调研 GLM-4-Flash 的 JSON 输出能力，确定 P5 的 schema 校验策略
...
- [x] **Task 1.1**: 编写内置热门路线表（10-15 条，含准海拔和 GCJ-02 坐标）
...
- [x] **Task 1.2**: 实现 resolveLocation(route) —— 含模糊匹配、高德 POI、海拔查询、坐标转换
...
- [x] **Task 2.1**: 实现 fetchWeather(lat, lon, elevation, date)
...
- [x] **Task 3.1**: 实现 calcSunEvents(lat, lon, date)
...
- [x] **Task 4.1**: 编写 gear-rules.js（季节×海拔×天数×纬度带）
...
- [x] **Task 4.2**: 编写 prompt.js（system prompt + 模板 + 降级模板）
...
- [x] **Task 5.1**: 串联全链路 + Promise.all 并行
...
- [x] **Task 5.2**: JSON schema 校验 + 分层降级
...
- [x] **Task 8.1**: 端到端测试（含非 happy path）
