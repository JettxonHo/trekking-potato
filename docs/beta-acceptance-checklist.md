...
| A6 | Fuzzy confirmation has cancellation/edit boundary and no early weather/context/AI side effects | `VERIFIED` | `npm run test:confirmation`, `npm run test:trip-flow` and `npm run test:beta-acceptance` | Confirmation/page contracts cover candidate rendering and editability; trip-flow covers RESET cancellation/token isolation; Beta acceptance proves fuzzy response and zero side effects before confirm. |
...
| A11 | Weather re-prepare keeps the old deterministic result visible during refresh and obtains a replacement query authority | `VERIFIED` | `npm run test:recovery`; `npm run test:beta-acceptance`; supplemental repeated-prepare probe | Recovery contract asserts `preparing + result` becomes local refresh and preserves the result; the public fixture probe observed two distinct server query IDs for repeated prepare calls. |
