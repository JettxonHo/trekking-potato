...
async function main() {
  ...
  assert(saveHistorySource.includes('historyError'), 'history save 失败必须只写入局部 historyError')
  assert((fetchAdviceSource.match(/this\._saveHistory\(historyParams,/g) || []).length === 3, 'normal advice、ordinary error 与 transport failure 都必须尝试保存 private history')
