...
async function main() {
  ...
  assert(pageSource.includes('confirmationInput: { date: params.date, level: params.level, days: params.days }'), 'prepare confirmation 必须经 reducer 保存当次 date/level/days 快照')
  ...
  assert(pageSource.includes('this._getAdviceService().confirm(params)'), 'confirm 调用必须经 getAdvice service')
  const candidateSelectStart = pageSource.indexOf('onCandidateSelect = (candidateId)')
  const candidateSelectEnd = pageSource.indexOf('onManualRouteTypeChange =', candidateSelectStart)
  const candidateSelectSource = pageSource.slice(candidateSelectStart, candidateSelectEnd)
  assert(candidateSelectSource.includes('const { candidates, confirmationInput } = this.state.tripFlow'), '候选和确认快照必须只从 reducer state 读取')
  assert(candidateSelectSource.includes('const base = result.data'), 'confirm 成功 base 必须先保留服务端解析的路线事实')
  assert(candidateSelectSource.includes('const historyParams = { ...params, route: params.route || base.route }'), 'confirm 成功后 historyParams 必须补入服务端 base.route')
  assert(candidateSelectSource.includes('this._showBaseAndFetchAdvice(base, result.queryId, historyParams, token)'), 'confirm base 必须只把本地 historyParams 与可信 queryId/token 交给 base→advice 流程')
  assert(!candidateSelectSource.includes('queryId: result.queryId'), 'confirm 本地 historyParams 不得保存 queryId')
  ...
  assert(pageSource.includes("onCandidateClose = () =>") && pageSource.includes("type: 'RESET'"), '取消候选必须经 reducer RESET 使旧 prepare/confirm 回调失效')
